...
{
    cv::Mat img1 = cv::imread("img.png");
    cv::Mat img2 = img1.clone();
    cv::GaussianBlur(img2, img2, cv::Size(15, 15), 10);
    double psnr = getPSNR(img1, img2);
    cv::Scalar mssim = getMSSIM(img1, img2);

    std::cout << "PSNR: " << psnr << std::endl;
    std::cout << "SSIM: " << mssim << std::endl;

    cv::Mat h;
    cv::hconcat(img1, img2, h);
